...
def check_drinks(drink_name, drink_ingredients):
    ...
    drink_type = "Mocktail" if set(drink_ingredients).isdisjoint(ALCOHOLS) else "Cocktail"
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for name, ingridients in DISH_CATEGORIES.items():
        if dish_ingredients.issubset(ingridients):
...
def tag_special_ingredients(dish):
    ...
    name, ingridients = dish
    alergic_ingredients = set(ingridients).intersection(SPECIAL_INGREDIENTS)

    return (name, alergic_ingredients)
